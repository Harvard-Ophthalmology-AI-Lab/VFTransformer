...
# sys.path.append('..')
import math as m
from TabTransformerTF.tabtransformertf_revised.models.embeddings import CEmbedding, NEmbedding
...
#         self.ln2 = tf.keras.layers.LayerNormalization()
    ...
    def call(self, inputs):
        x_td24 = self.td_encoder(inputs[1])
        if self.encoder.explainable:
            x, expl = self.encoder(inputs[0])
        else:
            x = self.encoder(inputs[0])

        layer_norm_cls = self.ln(x[:, 0, :])
        layer_norm_cls = self.final_ff(layer_norm_cls)
        layer_norm_cls = Concatenate()([layer_norm_cls, x_td24])
#         layer_norm_cls = self.ln2(layer_norm_cls)
